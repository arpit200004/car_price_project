...
DATA_FILE = BASE_DIR / "data/raw/used-cars-dataset-cardekho/cars_details_merges.csv.gz"
SYNTHETIC_FILE = BASE_DIR / "data/processed/synthetic_2024_cars.csv"
...
def load_market_data() -> pd.DataFrame:
    if not DATA_FILE.exists():
        raise FileNotFoundError(f"Missing raw data at {DATA_FILE}")
    ...
    try:
        ...
        print(f"Warning: Synthetic data not found at {SYNTHETIC_FILE}. Skipping.")
        raw = raw_main
    ...
    df = df[
        ...
        & df["model_year"].between(2000, 2024)
        & df["mileage"].between(5, 35)
        & df["engine_cc"].between(500, 6_500)
        & df["max_power"].between(25, power_cap)
        & df["max_torque"].between(25, torque_cap)
        & df["seats"].between(2, 9)
        & df["length"].between(2800, 5_500)
